...
public class NetworkTaichoGame : LoadBalancingClient
{
	...
	// [Set in Start() of UIManagerGame.cs]
	public TaichoGameGrid taichoGrid;
	...
	{
		return new NetworkTaichoPlayer(actorName, actorNumber, isLocal, actorProperties);
	}
	...
	public override void OnOperationResponse(OperationResponse operationResponse)
	{
		...
		case (byte)OperationCode.JoinRandomGame:
			...
			{
				string randomRoomName = "BewareTheInvisibleDragons-" +Random.Range(0,1000).ToString("D4");
				this.CreateTurnbasedRoom(randomRoomName);
			}
			break;
		}
	}
	...
	private void OnWebRpcResponse(WebRpcResponse response)
	{
		...
		{
			Debug.Log("WebRpcResponse for GetGameList :" + response.ToStringFull());
...
//			Debug.Log ("Disconnected!!! :(");
	...
	public void SaveBoardAsProperty()
	{
		...
		Hashtable taichoGridProps = taichoGrid.GetBoardAsCustomProperties();
		Debug.Log ("got this board property object for Taicho game grid" + taichoGridProps);
		Hashtable taichoTurnGridProps = taichoGrid.GetTurnCustomProperties();
		taichoGridProps.Merge (taichoTurnGridProps);

		taichoGridProps.Add("lt", this.lastTurnPlayer);  // "lt" is for "last turn" and contains the ID/actorNumber of the player who did the last one
		taichoGridProps.Add("t#", this.turnNumber);

		this.OpSetCustomPropertiesOfRoom(taichoGridProps, false);
		
		Debug.Log("saved board to props " + SupportClass.DictionaryToString(taichoGridProps));
	}
	...
	public void UpdateBoard() {
		...
		bool success = taichoGrid.SetBoardByCustomProperties(roomProps);
	...
	/// </summary>
	public void CreateTurnbasedRoom(string roomName)
	{
//		string newRoomName = this.NickName + "-" +Random.Range(0,1000).ToString("D4");    // for int, Random.Range is max-exclusive!
		Debug.Log("CreateTurnbasedRoom() will create: " + roomName);

		RoomOptions demoRoomOptions = new RoomOptions()
		{
			MaxPlayers = NetworkTaichoGame.MaxPlayers,
			CustomRoomPropertiesForLobby = NetworkTaichoGame.RoomPropsInLobby,
			EmptyRoomTtl = 10000,
			PlayerTtl = int.MaxValue
		};
		this.OpCreateRoom(roomName, demoRoomOptions, TypedLobby.Default);
	}
}
